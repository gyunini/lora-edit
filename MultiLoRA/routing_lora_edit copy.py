...
def apply_Routing_LoRA_Edit(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        # Clear GPU memory
        del layer_ks, cur_zs, targets, A, X, M, B_j
...
class RoutingLoRAHooks:
    ...
    OPTIMIZATION:
    - Delta_j = B_j @ A is pre-computed and stored on GPU
    - U_bank is stacked into a single tensor for vectorized score computation
    ...
    def __init__(
        ...
        # ===== PRE-COMPUTE Delta_j = B_j @ A and stack tensors =====
        print("[RoutingLoRAHooks] Pre-computing Delta matrices and stacking tensors...")
        
        self.Delta_stacked = {}  # {layer: (n_edits, d1, d0)}
        self.U_stacked = {}      # {layer: (n_edits, d0, r_route)}
        ...
        for layer in hparams.layers:
            ...
            A = A_matrices[layer].to(device)
            
            # Pre-compute all Delta_j = B_j @ A
            deltas = []
            for B in B_bank[layer]:
                delta = B.to(device) @ A  # (d1, d0)
                deltas.append(delta)
            
            # Stack into single tensor: (n_edits, d1, d0)
            self.Delta_stacked[layer] = torch.stack(deltas, dim=0)
            
            # Stack U matrices: need to pad if different r_route
            # Assuming all U_j have same shape (d0, r_route)
            U_list = [U.to(device) for U in U_bank[layer]]
            # Stack: (n_edits, d0, r_route)
            self.U_stacked[layer] = torch.stack(U_list, dim=0)
            
            print(f"  Layer {layer}: Delta_stacked {self.Delta_stacked[layer].shape}, "
                  f"U_stacked {self.U_stacked[layer].shape}")
        
        print(f"[RoutingLoRAHooks] Pre-computation done. Total edits per layer: {self.n_edits}")
        
    def _make_hook(self, layer: int):
        """Create a forward hook for a specific layer (OPTIMIZED)."""
        
        def hook_fn(module, input, output):
            """
            Hook function that adds routed delta to the output.
            
            OPTIMIZED: Uses pre-computed Delta matrices and vectorized operations.
            No Python loops in the hot path.
            """
            ...
            # Get pre-computed tensors (already on GPU)
            Delta = self.Delta_stacked[layer]  # (n_edits, d1, d0)
            U = self.U_stacked[layer]          # (n_edits, d0, r_route)
            n_edits = Delta.shape[0]
            d1 = Delta.shape[1]
            
            if self.hparams.routing_mode == "none":
                # Sum all deltas: (d1, d0)
                total_delta = Delta.sum(dim=0)
                # Apply: (d1, d0) @ (d0, n_tokens) -> (d1, n_tokens) -> (n_tokens, d1)
                delta_output = (total_delta @ inp_flat.T).T
            
            elif self.hparams.routing_mode == "hard":
                # Vectorized score computation
                # U: (n_edits, d0, r_route), inp_flat: (n_tokens, d0)
                # proj = inp_flat @ U -> (n_tokens, n_edits, r_route)
                scores = compute_routing_scores_vectorized(inp_flat, U)  # (n_tokens, n_edits)
                
                # Hard routing: select one edit per token
                selected = scores.argmax(dim=-1)  # (n_tokens,)
                
                # Vectorized delta application using gather/index_select
                # Delta: (n_edits, d1, d0)
                # inp_flat: (n_tokens, d0)
                # For each token i, we want Delta[selected[i]] @ inp_flat[i]
                
                # Approach: compute ALL delta outputs, then select
                # all_outputs[i, j] = Delta[j] @ inp_flat[i]
                # inp_flat: (n_tokens, d0) -> (n_tokens, 1, d0)
                # Delta: (n_edits, d1, d0) -> (1, n_edits, d1, d0)
                # einsum: 'td,ndo->tno' -> (n_tokens, n_edits, d1)
                all_outputs = torch.einsum('td,ndo->tno', inp_flat, Delta.transpose(-1, -2))
                # all_outputs: (n_tokens, n_edits, d1)
                
                # Select the correct edit for each token
                # selected: (n_tokens,) -> gather along dim 1
                selected_expanded = selected.unsqueeze(-1).unsqueeze(-1).expand(-1, 1, d1)
                delta_output = all_outputs.gather(1, selected_expanded).squeeze(1)  # (n_tokens, d1)
                ...
                # Compute ALL delta outputs at once
                # einsum: 'td,ndo->tno' -> (n_tokens, n_edits, d1)
                all_outputs = torch.einsum('td,ndo->tno', inp_flat, Delta.transpose(-1, -2))
                
                # Weighted sum: weights (n_tokens, n_edits), all_outputs (n_tokens, n_edits, d1)
                # einsum: 'tn,tno->to' -> (n_tokens, d1)
                delta_output = torch.einsum('tn,tno->to', weights, all_outputs)

...
class RoutingLoRAHooks:
    ...
    def __init__(
        ...
        # ===== PERFORMANCE-OPTIMIZED: Pre-load A and B_j to GPU for fast inference =====
        print("[RoutingLoRAHooks] Setting up performance-optimized routing (pre-loading to GPU)...")
        
        self.A_matrices = {}     # {layer: A (r, d0) on GPU}
        self.B_stacked = {}      # {layer: (n_edits, d1, r) on GPU} - stacked for vectorization
        self.U_stacked = {}      # {layer: (n_edits, d0, r_route) on GPU}
        self.n_edits = {}        # {layer: n_edits}
        
        # NEW: For CW-Edit
        self.t_stacked = {}      # {layer: (n_edits,) on GPU}
        self.tau_stacked = {}    # {layer: (n_edits,) on GPU}
        ...
        for layer in hparams.layers:
            ...
            # Stack B_j on GPU for vectorized operations
            B_list = [B.to(device) for B in B_bank[layer]]
            self.B_stacked[layer] = torch.stack(B_list, dim=0)  # (n_edits, d1, r) on GPU
            
            # Stack U matrices on GPU
            U_list = [U.to(device) for U in U_bank[layer]]
            self.U_stacked[layer] = torch.stack(U_list, dim=0)  # (n_edits, d0, r_route) on GPU
            ...
            if hparams.routing_mode == "cw" and router_stats_bank is not None:
                ...
                if len(stats_list) == n:
                    ...
                    self.t_stacked[layer] = torch.tensor(t_list, device=device, dtype=torch.float32)
                    self.tau_stacked[layer] = torch.tensor(tau_list, device=device, dtype=torch.float32)
                else:
                    # Fallback: use mu for t, floor for tau
                    print(f"Warning: stats mismatch for layer {layer}, using fallback")
                    self.t_stacked[layer] = torch.zeros(n, device=device, dtype=torch.float32)
                    self.tau_stacked[layer] = torch.full((n,), hparams.router_tau_floor, device=device, dtype=torch.float32)
    ...
    def _make_hook(self, layer: int):
        ...
        def hook_fn(module, input, output):
            ...
            # Get pre-loaded tensors (already on GPU)
            A = self.A_matrices[layer]  # (r, d0) on GPU
            B_stacked = self.B_stacked[layer]  # (n_edits, d1, r) on GPU
            U = self.U_stacked[layer]  # (n_edits, d0, r_route) on GPU
